...
export function PremiumCard({ planName = "Premium", price = "50", currency = "$/mth", onAction, className }: PremiumCardProps) {
  return (
    <div className={cn(
      "bg-[var(--surface-inverse)] rounded-[var(--radius-lg)] p-4.5 px-5 relative overflow-hidden shadow-[var(--shadow-md)]",
      className
    )}>
      <CardDecor seed={7} />
      <div className="text-xs font-semibold text-[var(--text-secondary)] uppercase tracking-[0.06em] mb-1 relative">Current plan</div>
      <div className="text-[1.5rem] text-[var(--text-inverse)] font-extrabold mb-3 relative">{planName} ✦</div>
      <div className="absolute top-4 right-5 bg-[var(--accent)] text-[var(--text-on-accent)] text-xs font-extrabold px-2.5 py-1 rounded-full">
